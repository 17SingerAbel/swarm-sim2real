...
def _hex_grid_positions(grid_size, node_spacing):
    """Return list of {sensor_id, x, y} for a staggered hex grid.

    Even columns: y = row * dy
    Odd  columns: y = row * dy + node_spacing / 2   (half-step offset)
    sensor_id = col * grid_size + row   (matches MATLAB node numbering)
    """
