...
MCMF_STAGE3_INJECTION_TIME = 5.0;             % Default-off short controlled simulation injection time

stage3_use_mcmf_env = getenv('MCMF_STAGE3_USE_MCMF');
if ENABLE_MCMF_STAGE3_TEST && any(strcmpi(stage3_use_mcmf_env, {'0', 'false', 'no', 'off', 'legacy'}))
    USE_MCMF_ASSIGNMENT = false;
end
if USE_MCMF_ASSIGNMENT
    stage3_assignment_mode = 'MCMF';
else
    stage3_assignment_mode = 'LEGACY';
end

stage3_sim_time_env = str2double(getenv('MCMF_STAGE3_SIMULATION_TIME'));
if ENABLE_MCMF_STAGE3_TEST && ~isnan(stage3_sim_time_env) && stage3_sim_time_env > 0
    simulation_time = stage3_sim_time_env;
end

stage3_injection_time_env = str2double(getenv('MCMF_STAGE3_INJECTION_TIME'));
if ENABLE_MCMF_STAGE3_TEST && ~isnan(stage3_injection_time_env) && stage3_injection_time_env > 0
...
params = struct('a', a, 'node_spacing', node_spacing, 'dt', dt, ...
    ...
    'ENABLE_MCMF_STAGE3_TEST', ENABLE_MCMF_STAGE3_TEST, ...
    'MCMF_STAGE3_INJECTION_TIME', MCMF_STAGE3_INJECTION_TIME);
...
assignment_events = struct('time', {}, 'algorithm', {}, 'calling_targets', {}, ...
    ...
    'assignment_step_runtime', {}, 'cost_matrix_runtime', {}, ...
    'mcmf_solver_runtime', {}, 'legacy_solver_runtime', {}, ...
    'selected_solver_runtime', {});
...
for t = 1:simulation_time/dt
    ...
    if ENABLE_MCMF_STAGE3_TEST && ~stage3_injection_done && current_time >= MCMF_STAGE3_INJECTION_TIME
        ...
        for forced_idx = 1:length(forced_targets)
            ...
            forced_intercept_point = max([0, 0], min([wsn_width, wsn_height], forced_intercept_point));
            forced_intercept_points(forced_idx, :) = forced_intercept_point;

            active_trackers{forced_tid} = forced_trackers;
            interceptor_process_state{forced_tid} = 'PENDING_SELECTION';
            interceptor_process_delay(forced_tid) = 0;
            interceptor_call_triggered(forced_tid) = false;
            interceptor_valid_bidders{forced_tid} = setdiff((1:num_nodes)', vertcat(forced_tracker_sets{:}));
            interceptor_bids(:, forced_tid) = 0;

            for role_idx = 1:length(forced_trackers)
                tracker_id = forced_trackers(role_idx);
                old_state = sensor_states{tracker_id};
                old_role = sensor_roles{tracker_id};
                sensor_states{tracker_id} = SENSOR_STATES.TRACKING;
                if role_idx == 1
                    sensor_roles{tracker_id} = SENSOR_ROLES.PRIMARY_TRACKER;
                else
                    sensor_roles{tracker_id} = SENSOR_ROLES.SECONDARY_TRACKER;
                end
                proactive_targets{tracker_id} = [];
                sensor_detection_times(tracker_id, forced_tid) = current_time;
                sensor_ekf_states{tracker_id, forced_tid} = [forced_target_position'; forced_velocity'];
                sensor_P_matrices{tracker_id, forced_tid} = diag([0.01, 0.01, 0.001, 0.001]);
                sensor_P_trace_history{tracker_id, forced_tid} = [sensor_P_trace_history{tracker_id, forced_tid}; ...
                    current_time, trace(sensor_P_matrices{tracker_id, forced_tid})];
                logStateTransition(tracker_id, old_state, SENSOR_STATES.TRACKING, old_role, sensor_roles{tracker_id}, ...
                    current_time, '[MCMF_TEST_INJECTION] Forced tracker for controlled simulation');
            end

            interceptor_process_data{forced_tid} = struct('predictor_id', forced_trackers(1), ...
                'target_id', forced_tid, ...
                'loss_time', current_time + 4.0, ...
                'intercept_point', forced_intercept_point, ...
                'target_position', forced_target_position, ...
                'loss_point', forced_loss_point);
            global_interceptor_data{forced_tid} = interceptor_process_data{forced_tid};

            new_event = struct('time', current_time, 'target_id', forced_tid, ...
                'predictor_sensor', forced_trackers(1), ...
                'selected_sensors', [], ...
                'intercept_point', forced_intercept_point, ...
                'loss_time', current_time + 4.0);
        ...
        stage3_injection_done = true;
        stage3_injection_info = struct('enabled', true, ...
        ...
        if ~isempty(current_active_trackers)
            ...
            switch interceptor_process_state{target_id}
                ...
                case 'PENDING_SELECTION'
                    ...
                    assignment_step_timer = tic;
                    cost_matrix_timer = tic;
                    [available_sensors, assignment_cost_matrix] = buildAssignmentCostMatrixForCallingTargets( ...
                        ...
                        wsn_width, wsn_height);
                    cost_matrix_runtime = toc(cost_matrix_timer);
                    ...
                    selected_solver_runtime = toc(selected_solver_timer);
                    if USE_MCMF_ASSIGNMENT
                        mcmf_solver_runtime = selected_solver_runtime;
                        legacy_solver_runtime = NaN;
                    else
                        mcmf_solver_runtime = NaN;
                        legacy_solver_runtime = selected_solver_runtime;
                    end
                    assignment_step_runtime = toc(assignment_step_timer);
                    ...
                    fprintf(fid, ['[t=%5.2f][ASSIGN_RUNTIME] algorithm=%s targets=[%s] candidates=%d ' ...
                        'cost_matrix=%.6f selected_solver=%.6f mcmf_solver=%.6f legacy_solver=%.6f ' ...
                        'assignment_step=%.6f dt=%.3f selected_solver_dt_pct=%.3f assignment_step_dt_pct=%.3f\n'], ...
                        current_time, assignment_info.algorithm, num2str(all_calling_targets'), ...
                        length(available_sensors), cost_matrix_runtime, selected_solver_runtime, ...
                        mcmf_solver_runtime, legacy_solver_runtime, assignment_step_runtime, dt, ...
                        100 * selected_solver_runtime / dt, 100 * assignment_step_runtime / dt);
                    ...
                    new_assignment_event = struct( ...
                        ...
                        'assignment_step_runtime', assignment_step_runtime, ...
                        'cost_matrix_runtime', cost_matrix_runtime, ...
                        'mcmf_solver_runtime', mcmf_solver_runtime, ...
                        'legacy_solver_runtime', legacy_solver_runtime, ...
                        'selected_solver_runtime', selected_solver_runtime);
                    ...
                    if ENABLE_MCMF_STAGE3_TEST && stage3_injection_info.injected && isempty(stage3_assignment_snapshot) && ...
                        ...
                        stage3_selected_sensors = [];
                        for snapshot_idx = 1:length(selected_assignments)
                            stage3_selected_sensors = [stage3_selected_sensors; selected_assignments{snapshot_idx}(:)];
                        end
                        stage3_selected_sensors = unique(stage3_selected_sensors, 'stable');
                        stage3_assignment_snapshot = struct( ...
                            'time', current_time, ...
                            'event_index', length(assignment_events), ...
                            'algorithm', assignment_info.algorithm, ...
                            'calling_targets', all_calling_targets', ...
                            'selected_sensors', stage3_selected_sensors', ...
                            'active_trackers_target_1', active_trackers{1}', ...
                            'active_trackers_target_2', active_trackers{2}', ...
                            'active_trackers_at_assignment', {active_trackers}, ...
                            'sensor_states', {sensor_states(stage3_selected_sensors)}, ...
                            'sensor_roles', {sensor_roles(stage3_selected_sensors)}, ...
                            'proactive_targets', {proactive_targets(stage3_selected_sensors)}, ...
                            'node_positions', node_positions(stage3_selected_sensors, :));
                    end
