...
% V46_Yeqi: Added USE_GLOBAL_ASSIGNMENT to support A/B testing between the
...
sim_root = fileparts(script_dir);
logs_dir = fullfile(sim_root, 'generated', 'logs');
if ~exist(logs_dir, 'dir'), mkdir(logs_dir); end
log_filename  = fullfile(logs_dir, sprintf('wsn_log_%s.log', run_timestamp));
mat_filename  = fullfile(logs_dir, sprintf('wsn_data_%s.mat', run_timestamp));
global fid;
...
fprintf(fid, '[t= 0.00][INIT] USE_GLOBAL_ASSIGNMENT: %d (%s)\n', USE_GLOBAL_ASSIGNMENT, assignment_mode_label);
...
assignment_events = struct('time', {}, 'algorithm', {}, 'calling_targets', {}, ...
    ...
    'legacy_comparison_mode', {}, 'legacy_is_valid_two_target_minimax_case', {}, ...
    'mcmf_worst_team_cost', {}, 'legacy_worst_team_cost', {}, ...
    'comparison_delta_total_cost', {}, 'comparison_delta_worst_team_cost', {}, ...
            ...
            switch interceptor_process_state{target_id}
                ...
                case 'PENDING_SELECTION'
                    ...
                    [all_calling_targets, reselection_events] = expandCallingTargetsForAssignment( ...
                        ...
                        wsn_width, wsn_height, MAX_ACTIVE_TRACKERS, USE_GLOBAL_ASSIGNMENT);
                    ...
                    mcmf_assigned_mask = mcmf_info.assigned_slots_per_target > 0;
                    if any(mcmf_assigned_mask)
                        mcmf_worst_team_cost = max(mcmf_info.assignment_costs_per_target(mcmf_assigned_mask));
                    else
                        mcmf_worst_team_cost = 0;
                    end

                    legacy_assigned_mask = legacy_info.assigned_slots_per_target > 0;
                    if any(legacy_assigned_mask)
                        legacy_worst_team_cost = max(legacy_info.assignment_costs_per_target(legacy_assigned_mask));
                    else
                        legacy_worst_team_cost = 0;
                    end

                    fprintf(fid, ['[t=%5.2f][ASSIGN_COMPARE] targets=[%s] candidates=%d | ' ...
                        'MCMF assigned=%d total_cost=%.3f worst_team_cost=%.3f | ' ...
                        'LEGACY assigned=%d total_cost=%.3f worst_team_cost=%.3f mode=%s valid_two_target_minimax=%d | ' ...
                        'delta_total(MCMF-LEGACY)=%.3f delta_worst(MCMF-LEGACY)=%.3f\n'], ...
                        current_time, num2str(all_calling_targets'), length(available_sensors), ...
                        mcmf_info.assigned_slots_total, mcmf_info.total_cost, mcmf_worst_team_cost, ...
                        legacy_info.assigned_slots_total, legacy_info.total_cost, legacy_worst_team_cost, ...
                        legacy_info.comparison_mode, double(legacy_info.is_valid_two_target_minimax_case), ...
                    ...
                    new_assignment_event = struct( ...
                        ...
                        'legacy_comparison_mode', legacy_info.comparison_mode, ...
                        'legacy_is_valid_two_target_minimax_case', legacy_info.is_valid_two_target_minimax_case, ...
                        'mcmf_worst_team_cost', mcmf_worst_team_cost, ...
                        'legacy_worst_team_cost', legacy_worst_team_cost, ...
                        'comparison_delta_total_cost', mcmf_info.total_cost - legacy_info.total_cost, ...
                        'comparison_delta_worst_team_cost', mcmf_worst_team_cost - legacy_worst_team_cost, ...
